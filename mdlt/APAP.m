...
gamma = 0.001; % Normalizer for Moving DLT. (0.0015-0.1 are usually good numbers).
sigma = 4;  % Bandwidth for Moving DLT. (Between 8-12 are good numbers).   
scale = 1;    % Scale of input images (maybe for large images you would like to use a smaller scale).

%------------------
% Images to stitch.
%------------------
path1 = 'images/73_A.jpg';
path2 = 'images/73_B.jpg';
...
apap_mdlt = imageblending(warped_img1,warped_img2);
fprintf('done (%fs)\n',toc);
figure;
imshow(apap_mdlt);
title('As-Projective-As-Possible Image Stitching with Moving DLT');
fprintf('> Finished!.\n');
imwrite(apap_mdlt,'APAP.jpg');
